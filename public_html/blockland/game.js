...
class Game{
	...
	playerControl(forward, turn){
		...
		if (forward>0.3){
			if (this.player.action!='Walking' && this.player.action!='Goofy Running') this.player.action = 'Walking';
		}else if (forward<-0.3){
			if (this.player.action!='Drunk Walk Backwards') this.player.action = 'Drunk Walk Backwards';
		}else{
			forward = 0;
	...
	animate() {
		...
		if (this.player.action=='Walking'){
			...
			if (elapsedTime>1000 && this.player.motion.forward>0){
				this.player.action = 'Goofy Running';
			}
		}
	...
	move(dt){
		...
		if (!blocked){
			if (this.motion.forward>0){
				const speed = (this.action=='Goofy Running') ? 500 : 150;
...
class SpeechBubble{
	constructor(game, msg, size=1){
		this.config = { font:'Helvetica', size:24, padding:10, colour:'#222', width:256, height:256 };

...
class Game{
	constructor(){
		...
		const game = this;
		this.anims = ['Walking', 'Walking Backwards', 'Right Turn', 'Drunk Walk Backwards', 'Pointing Gesture', 'Goofy Running',];
	...
	init() {
		...
		loader.load( `${this.assetsPath}fbx/people/Happy.fbx`, function ( object ) {
    ...
    movePlayer(dt){
		...
		if (!blocked){
			if (this.player.move.forward>0){
				const speed = (this.player.action=='Goofy Running') ? 400 : 150;
    ...
    playerControl(forward, turn){
		...
		if (forward>0.3){
			if (this.player.action!='Walking' && this.player.action!='Goofy Running') this.action = 'Walking';
		}else if (forward<-0.3){
			if (this.player.action!='Drunk Walk Backwards') this.action = 'Drunk Walk Backwards';
		}else{
			forward = 0;
			if (Math.abs(turn)>0.1){
                if (turn > 0 && this.player.action != 'Turn Left') this.action = 'Turn Left';
                if (turn < 0 && this.player.action != 'Right Turn') this.action = 'Right Turn';
			// if (Math.abs(turn)>0.1){
			// 	if (this.player.action != 'Right Turn') this.action = 'Right Turn';
